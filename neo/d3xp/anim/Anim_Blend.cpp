	...
	while( frame != to )
	{
		...
		while( index < end )
		{
			...
			switch( command.type )
			{
				...
				case FC_FX:
				{
					idEntityFx::StartFx( command.string->c_str(), nullptr, nullptr, ent, true );
					break;
				}
				case FC_SKIN:
				{
...
int idAnimator::GetChannelForJoint( jointHandle_t joint ) const
{
	if( !modelDef )
	{
		gameLocal.Error( "idAnimator::GetChannelForJoint: nullptr model" );
		return -1;
	}
...
{
	return animationLib.GetAnim( fileName, nullptr );
}
...
idRenderModel* idGameEdit::ANIM_CreateMeshForAnim( idRenderModel* model, const char* classname, const char* animname, int frame, bool remove_origin_offset )
{
	...
	else
	{
		...
		md5anim = animationLib.GetAnim( animname, nullptr );
		offset.Zero();
	}
