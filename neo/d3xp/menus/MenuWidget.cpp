...
{
	if( !verify( children.Find( widget ) == nullptr ) )
	{
...
Descend through GetFocus() calls until you reach a nullptr focus.  The terminating widget is the
...
{
	if( !verify( observers.Find( observer ) == nullptr ) )
	{
		return;
	}
